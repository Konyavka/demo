...
Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ДобавитьКоманду(ТаблицаКоманд, "Приход денежных средств с отборами (таб.1.6.1)", "1", "ОткрытиеФормы", Истина, "");
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ДоступноеПодразделение = ПолучитьДоступноеПодразделениеПользователя();
	...
	Иначе
		ПараметрПодразделение.Использхвоание = ЛОжь;
	КонецЕсли;	
	...
	СхемаКомпоновкиДанных.НаборыДанных.ТекущийМесяцЦел.Элементы[1].Запрос = 
		ПолучитьТекстЗапросаПлан("&НачПериода", "&КонПериода", "СуммаТекПлан");
	...
	СхемаКомпоновкиДанных.НаборыДанных.ТекущийМесяцЦел.Элементы[6].Запрос = 
		ПолучитьТекстЗапросаПлан("&НачПериодаСлед", "&КонПериодаСлед", "СуммаСледПлан");
...
Функция ПолучитьТекстЗапросаОплаты(ИмяПараметраНачПериода, ИмяПараметраКонПериода, ИмяПараметраСумма, ИмяПараметраСуммаНовый)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	РасчетыСКлиентами.ОбъектРасчетов.Объект.Партнер КАК Партнер
	               ...
	               |	И РасчетыСКлиентами.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента)
	               |	И ТаблицаПриемка.Ссылка.Подразделение В ИЕРАРХИИ(&Подразделение)
	               |{ГДЕ
	               |	(ТаблицаПриемка.Значение = &Приемка) КАК ВидПриемки}
	               |
	               |СГРУППИРОВАТЬ ПО
...
Функция ПолучитьТекстЗапросаПлан(ИмяПараметраНачПериода, ИмяПараметраКонПериода, ИмяПараметраСумма)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ТаблицаАналитикаСтатейБюджетов.Аналитика2 КАК ВидНоменклатуры,
	               |	СУММА(ТаблицаОборотыПоСтатьям.Сумма) / 1000 КАК СуммаТекПлан
	               |ИЗ
	               ...
	               |{ГДЕ
	               |	(ТаблицаАналитикаСтатейБюджетов.Аналитика5 = &Приемка) КАК ВидПриемки}
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ТаблицаАналитикаСтатейБюджетов.Аналитика2,
...
Функция ПолучитьДоступноеПодразделениеПользователя()
	...
	Если НЕ ЗначениеЗаполнено(ПодразделениеПользователя) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Для текущего пользователя не указано подразделение! Отчет сформирован не будет.");
		
		Возврат Справочники.СтруктураПредприятия.ПустаяСсылка();
	КонецЕсли;
	
	ПодразделениеЭнергия = Справочники.СтруктураПредприятия.НайтиПоКоду("00-000001");
	ПодразделениеИндикатор = Справочники.СтруктураПредприятия.НайтиПоКоду("00-000002");
	ПодразделениеОтделПроизводства = Справочники.СтруктураПредприятия.НайтиПоКоду("ЦУ-000006");
